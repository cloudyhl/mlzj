...
public class HbaseServiceImpl implements HbaseService {
    ...
    public void insert(UserEntity user) {
        try {
            //获取数据库中的表
            Table table = null;
            table = hbaseConnection.getTable(TableName.valueOf("mydemo:user"));
            //准备一行数据
...
//        admin.createNamespace(NamespaceDescriptor.create("mydemo").build());
//        TableName name = TableName.valueOf("mydemo:userinfos");
        TableDescriptorBuilder tableDescriptorBuilder = TableDescriptorBuilder.newBuilder(TableName.valueOf("mydemo:user"));
    ...
    public UserEntity findByRowKey(String rowKey) {
        ...
        try {
            ...
            userEntity.setUserId(rowKey);
            userEntity.setUserName(new String(result.getValue("base".getBytes(), "username".getBytes())));
    ...
    public List<UserEntity> findAll(){
        ...
        try {
            ...
            while ((result =rs.next())!= null){
                UserEntity userEntity = new UserEntity();
                userEntity.setUserId(new String(result.getRow()));
                userEntity.setUserName(new String(result.getValue("base".getBytes(), "username".getBytes())));
                userEntity.setBirthday(new String(result.getValue("base".getBytes(), "birthday".getBytes())));
                list.add(userEntity);
            }
